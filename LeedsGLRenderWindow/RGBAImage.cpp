...
bool RGBAImage::Resize(unsigned const int Width, unsigned const int Height)
    { // Resize()
...
RGBAValue * RGBAImage::operator [](unsigned const int rowIndex)
    { // row [] index operator
...
const RGBAValue * RGBAImage::operator [](unsigned const int rowIndex) const
    { // row [] index operator
    // use pointer arithmetic to compute the row beginning
    return block+(rowIndex*width);
    } // [] row index operator
